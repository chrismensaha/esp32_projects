...
static void vButtonISR(void){
    ...
    xTaskNotifyFromISR(buttonTask,buttonEvent,eSetValueWithOverwrite,&pxHigherPriorityTaskWoken);
...
static void vButtonReleased(void* pvParameter){
    ...
    for (;;){
        if (xTaskNotifyWait(0x00,0xFFFFFFFF,&serialCheck,portMAX_DELAY)==pdPASS){
...
void setup(){ 
    ...
    xTaskCreate(vButtonReleased,"Button Released",2048,NULL,1,&buttonTask);
